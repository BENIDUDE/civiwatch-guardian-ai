...
export const Card = ({ title, content, extendedTitle, extendedContent, lang }) => {
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 3000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', boxSizing: 'border-box' }} onClick={() => setIsExpanded(false)}>
          <div style={{ backgroundColor: '#0d1117', border: '1px solid #30363d', borderRadius: '16px', padding: '40px 20px', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }} onClick={e => e.stopPropagation()}>
            ...
            <h3 style={{ color: '#3498db', fontSize: '1.5rem', marginTop: 0, marginBottom: '20px', textAlign: isEn ? 'left' : 'right', direction: isEn ? 'ltr' : 'rtl' }}>{extendedTitle || title}</h3>
            {extendedContent}
          </div>
        </div>
      )}
...
export const Navbar = ({ lang, setLang }) => {
  ...
  return (
    <nav style={navStyle}>
      <Link to="/" style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#fff', textDecoration: 'none', whiteSpace: 'nowrap' }}>
      ...
      {!isMobile ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <ul style={{ display: 'flex', listStyle: 'none', margin: 0, padding: 0, gap: '15px', alignItems: 'center' }}>
            <li 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <button 
            ...
            style={{ background: 'none', border: 'none', color: '#fff', fontSize: '1.8rem', cursor: 'pointer', padding: 0 }}
          >
            {mobileMenuOpen ? '✕' : '☰'}
          </button>
          ...
          {mobileMenuOpen && (
            <div style={{ position: 'absolute', top: '100%', left: 0, width: '100%', backgroundColor: '#0d1117', borderBottom: '1px solid #30363d', padding: '20px 5%', display: 'flex', flexDirection: 'column', gap: '10px', boxShadow: '0 10px 20px rgba(0,0,0,0.5)', boxSizing: 'border-box' }}>
              <div style={{ borderBottom: '1px solid #30363d', paddingBottom: '10px', marginBottom: '10px' }}>
                <span style={{ color: '#fff', fontWeight: 'bold', display: 'block', marginBottom: '10px' }}>{isEn ? 'Solutions' : 'פתרונות'}</span>
                <div style={{ display: 'flex', flexDirection: 'column', paddingLeft: isEn ? '15px' : 0, paddingRight: isEn ? 0 : '15px', gap: '8px' }}>
                  <Link to="/solutions/social" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Social Networks' : 'רשתות חברתיות'}</Link>
                  <Link to="/solutions/marketplace" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'AI Marketplace' : 'זירת AI'}</Link>
                  <Link to="/solutions/governments" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Governments' : 'ממשלות'}</Link>
                  <Link to="/solutions/ngos" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'NGOs' : 'עמותות'}</Link>
                  <Link to="/solutions/genai" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Generative AI' : 'בינה מלאכותית יוצרת'}</Link>
                  <Link to="/solutions/vr" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Virtual Reality' : 'מציאות מדומה'}</Link>
                  <Link to="/solutions/hitl" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Human-in-the-Loop' : 'בקרת מומחים אנושית'}</Link>
                </div>
              </div>
              
              <Link to="/company" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Company' : 'החברה'}</Link>
              <Link to="/partners" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Partners' : 'שותפים'}</Link>
              <Link to="/market-strategy" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Strategy' : 'אסטרטגיה'}</Link>
              <Link to="/pricing" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Pricing' : 'תמחור'}</Link>
              <Link to="/contact" style={linkStyle} onClick={() => setMobileMenuOpen(false)}>{isEn ? 'Contact Us' : 'צור קשר'}</Link>
              
              <div style={{ marginTop: '15px' }}>
                {session ? (
                  <Link to="/dashboard" style={{ textDecoration: 'none' }} onClick={() => setMobileMenuOpen(false)}>
                    <Button label={isEn ? 'Dashboard' : 'לוח בקרה'} style={{ width: '100%', backgroundColor: '#10b981', color: '#022c22' }} />
                  </Link>
                ) : (
                  <Link to="/login" style={{ textDecoration: 'none' }} onClick={() => setMobileMenuOpen(false)}>
                    <Button label={isEn ? 'Platform Login' : 'כניסה למערכת'} style={{ width: '100%' }} />
                  </Link>
                )}
              </div>
            </div>
...
export const Footer = ({ lang }) => {
  ...
  return (
    <footer style={{ backgroundColor: '#0d1117', padding: '40px 5% 20px', borderTop: '1px solid #30363d', color: '#8b949e', zIndex: 10, position: 'relative', marginTop: 'auto', direction: isEn ? 'ltr' : 'rtl', fontSize: '0.85rem', boxSizing: 'border-box' }}>
      
      {/* NEWSLETTER & SOCIALS SECTION */}
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px', gap: '40px' }}>
        
        {/* Newsletter Signup */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', flex: '1 1 300px', maxWidth: '500px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', width: '100%' }}>
            <input 
              ...
              style={{ flex: '1 1 200px', backgroundColor: 'transparent', border: '1px solid #30363d', color: '#c9d1d9', padding: '10px 20px', borderRadius: '25px', outline: 'none', minWidth: '0' }}
            />
            <button 
              style={{ flex: '0 1 auto', backgroundColor: '#1f6feb', color: '#fff', border: 'none', padding: '10px 25px', borderRadius: '25px', fontWeight: 'bold', cursor: 'pointer', transition: 'background-color 0.2s', whiteSpace: 'nowrap' }} 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '15px', flex: '1 1 200px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '10px', textAlign: 'center', borderTop: '1px solid #30363d', paddingTop: '20px', lineHeight: '1.8' }}>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'Privacy Policy' : 'מדיניות פרטיות'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'CCPA Policy' : 'מדיניות CCPA'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'Security Policy' : 'מדיניות אבטחה'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'Data Policy' : 'מדיניות נתונים'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'Terms of Use' : 'תנאי שימוש'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <Link to="/coming-soon" style={{ color: '#8b949e', textDecoration: 'none', whiteSpace: 'nowrap' }} onMouseOver={e=>e.target.style.color='#fff'} onMouseOut={e=>e.target.style.color='#8b949e'}>{isEn ? 'Service Level Agreement' : 'אמנת שירות (SLA)'}</Link> <span style={{ opacity: 0.5 }}>|</span>
        <span style={{ whiteSpace: 'nowrap' }}>Copyright CiviWatch Guardian AI 2026. All Right Reserved.</span> <span style={{ opacity: 0.5 }}>|</span>
        <span style={{ whiteSpace: 'nowrap' }}>By Benjamin Michaeli</span>
      </div>
    </footer>
  );
};